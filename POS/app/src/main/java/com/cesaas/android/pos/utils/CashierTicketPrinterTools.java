 ...
 * ================================================
 */
public class CashierTicketPrinterTools {
     ...
     */
    public static void printLattice(Context context, LatticePrinter latticePrinter, LatticePrinterOrderBean printerBean,boolean isRefund) {
        String mediumSpline = "";
...
//                IPrint.Gravity.CENTER);
        ...
        int sizeTitle = largeSize - CashierTicketPrinterTools.length(title);
        ...
        if(isRefund==true){
            String time = "退款时间："+AbDateUtil.getCurrentDate("yyyy-MM-dd HH:mm:ss");
        ...
        latticePrinter.printText(name1 + "\n", LatticePrinter.FontFamily.SONG, LatticePrinter.FontSize.MEDIUM,
                LatticePrinter.FontStyle.BOLD);
        if(isRefund==true){
            String price1 = "退款方式:"+printerBean.getPayTitleName();
            latticePrinter.printText(price1 + "\n", LatticePrinter.FontFamily.SONG,
                    LatticePrinter.FontSize.MEDIUM, LatticePrinter.FontStyle.BOLD);
        }else{
            String price1 = "收款方式:"+printerBean.getPayTitleName();
            latticePrinter.printText(price1 + "\n", LatticePrinter.FontFamily.SONG,
                    LatticePrinter.FontSize.MEDIUM, LatticePrinter.FontStyle.BOLD);
        }

        String marchent1 = "实付金额:"+printerBean.getOriginalPrice();
        latticePrinter.printText(marchent1 + "\n", LatticePrinter.FontFamily.SONG,
                LatticePrinter.FontSize.MEDIUM, LatticePrinter.FontStyle.BOLD);

        String time1 = "实收金额:"+printerBean.getDiscountPrice();
        ...
        latticePrinter.printText(heji + "\n", LatticePrinter.FontFamily.SONG,
                LatticePrinter.FontSize.MEDIUM, LatticePrinter.FontStyle.BOLD);
        if(isRefund==true){
            String totalPrice = "退款总额："+ -printerBean.getTotalPrice();
            latticePrinter.printText(totalPrice + "\n", LatticePrinter.FontFamily.SONG,
                    LatticePrinter.FontSize.MEDIUM, LatticePrinter.FontStyle.BOLD);
        }else{
            String totalPrice = "支付总额："+printerBean.getTotalPrice();
            latticePrinter.printText(totalPrice + "\n", LatticePrinter.FontFamily.SONG,
                    LatticePrinter.FontSize.MEDIUM, LatticePrinter.FontStyle.BOLD);
        }
