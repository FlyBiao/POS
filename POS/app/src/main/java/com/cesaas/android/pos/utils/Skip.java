...
public class Skip {
	...
	public static void mRefundActivityResults(Activity context, Class<?> cls, int requestCode,String TraceAuditNumber) {
		Intent intent = new Intent(context, cls);
		intent.setFlags(Intent.FLAG_ACTIVITY_NO_HISTORY);
		intent.putExtra("mRefundResult","100");
		intent.putExtra("TraceAuditNumber",TraceAuditNumber);
		context.startActivityForResult(intent, requestCode);
		context.overridePendingTransition(R.anim.activity_slide_in_right, R.anim.activity_no_anim);
	}
